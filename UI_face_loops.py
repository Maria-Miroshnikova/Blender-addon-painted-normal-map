...
from bpy.props import FloatProperty, StringProperty, EnumProperty, PointerProperty, BoolProperty, IntVectorProperty
...
# 2) Не работает на слишком маленьких гранях
# 3) Только для моделей, симметричных по oX и расположенно центром в 0 по X
# ! разрез по может быть кольцом граней
# 5) Возможно, будет плохо работать на моделях с утолщением, рожками и других, где есть узкие расстояния между гранями
def make_symmetry_dictionary_by_median_similarity_with_central_strip(bm: BMesh):
    '''
    Функция устанавливает соответствие между индексами симметричных граней
    Возвращает словарь (индекс грани : индекс симметричной грани)
    Ключей столько же, сколько граней ! не половина
    См. ограничения функции!!
    '''
    symm_dict = {} # face_id1 : face_id2, face_id2 : face_id1, // face_id:face_id если грань на центральной полосе
    ...
    for face in bm.faces:
        median: Vector = face.calc_center_median()
        #if math.isclose(median.x, 0, rel_tol=rel_tol):
        if is_close_float(median.x, 0, rel_tol):
            symm_dict[face.index] = face.index
            face.select = True
            print(face.index)
        elif (median.x > 0):
            id_median_x_positive.append((face.index, median))
    ...
    for tuple1 in id_median_x_negative:
        ...
        for tuple2 in id_median_x_positive:
            ...
            #y_close = math.isclose(median_symm_expected1.y, median2.y, rel_tol=rel_tol)
            #z_close = math.isclose(median_symm_expected1.z, median2.z, rel_tol=rel_tol)
            x_close = is_close_float(median_symm_expected1.x, median2.x, rel_tol)
            y_close = is_close_float(median_symm_expected1.y, median2.y, rel_tol)
            z_close = is_close_float(median_symm_expected1.z, median2.z, rel_tol)
            if x_close and y_close and z_close:
                min_index = index2
                break
        #if (min_index < 0):
        #    print("problem")
        if (min_index == -1):
            print("face " + str(index1) + " " + str(median1))
            print(math.isclose(median1.x, 0, rel_tol=rel_tol))
       # assert(min_index >= 0)
        symm_dict[index1] = min_index
        symm_dict[min_index] = index1
    return symm_dict
...
def delete_edges_from_layer(mesh_obj: Object, bm: BMesh, layer_name: str, edges_id: List[int]):
    ...
    for e in edges_id:
        bm.edges[e][edge_group_layer] = NOT_BORDER_EDGE
...
######################################
#############################################################################################################################################################
# -- эти функции скорее всего будут модифицированы
...
def strokes_nocross_inside_border_partitial_with_layers(name: str, index: int, z_coord: int, start_loop: BMLoop,
                                                        bm: BMesh, visited_faces_id: set, Z_STEP: float,
                                                        COL_NAME: str, accessable_faces_id: Set[int], layer_dict: dict):
    '''
...
def strokes_nocross_symmetry_with_visited_recording_partitial_with_layers(name: str, index: int, z_coord: int,
                                                                          result: List[Tuple[List[BMesh], List[BMLoop], int]],
                                                                          symm_dict: dict, bm: BMesh, visited_faces_id: set,
                                                                          Z_STEP: float, COL_NAME: str, layer_dict: dict):
    '''
...
def strokes_nocross_inside_border_with_optional_symmetry_with_layers(name: str, index: int, z_coord: int, start_loop: BMLoop,
                                                                     bm: BMesh, visited_faces_id: set, Z_STEP: float, COL_NAME: str,
                                                                     accessable_faces_id: Set[int], layer_dict: dict, with_symmetry: bool, symm_dict: dict = {}):
    '''
    ...
    visited_faces_id, index, z_coord, result = strokes_nocross_inside_border_partitial_with_layers(name, index, z_coord, start_loop,
                                                                                                   bm, visited_faces_id, Z_STEP, COL_NAME,
                                                                                                   accessable_faces_id, layer_dict)

    # вызов симметричного отображения
    if (with_symmetry):
        visited_faces_id, index, z_coord = strokes_nocross_symmetry_with_visited_recording_partitial_with_layers(name, index, z_coord, result,
                                                                                                                 symm_dict, bm, visited_faces_id,
                                                                                                                 Z_STEP, COL_NAME, layer_dict)
...
def test_loops_for_loop_nocross_inside_borders_auto_visited(MESH_NAME_BASE: str, INDEX_START: int, Z_STEP: float, COL_NAME: str,
                                                            Z_COORD_START: int, layer_name: str, layer_file_name: str, symm_file_name: str, with_symmetry: bool, with_borders: bool):
    # --- --- --- --- --- prepare
     #--- EDIT MODE!
    ...
    if (with_borders):
        ...
        accessable_faces_id = [face.index for face in bm.faces]

    (visited_faces_id, index, z_coord) = strokes_nocross_inside_border_with_optional_symmetry_with_layers(MESH_NAME_BASE, INDEX_START, Z_COORD_START,
                                                                                                          starting_loop, bm, visited_faces_id, Z_STEP,
                                                                                                          COL_NAME, accessable_faces_id, layers_dict, with_symmetry, symm_dict)
...
def auto_strokes_nocross_inside_borders_with_optional_symmetry_with_layers(bm: BMesh, start_loop: BMLoop, Z_STEP: float, COL_NAME: str,
                                                                           MESH_NAME_BASE: str, MESH_NAME_IDX_START: int,
                                        Z_COORD_START: int, layer_name: str, border_edges_id: Set[id], accessable_faces_id: Set[int],
                                        layer_dict: dict, only_inside_border: bool, with_border: bool, with_symmetry: bool, symm_dict: dict = {}):
    '''
    ...
    (visited_faces_id, index, z_coord) = strokes_nocross_inside_border_with_optional_symmetry_with_layers(name, index, z_coord, start_loop,
                                                                                                          bm, visited_faces_id, Z_STEP, COL_NAME,
                                                                                                          accessable_faces_id, layer_dict,  with_symmetry, symm_dict)
    ...
    # выбор среди не граничных луп!
        ...
        if (loop_next == None):
            ...
            continue
        if (with_border): # else - просто не пересчитываем их
    ...
    # построить в отдельный strokemesh нужные вершины и цепь
        (visited_faces_id, index, z_coord) = strokes_nocross_inside_border_with_optional_symmetry_with_layers(name, index, z_coord, loop_next,
                                                                                                              bm, visited_faces_id, Z_STEP, COL_NAME,
                                                                                                              accessable_faces_id, layer_dict, with_symmetry, symm_dict)
...
def test_auto_strokes_nocross_inside_borders_with_layer(Z_STEP: float, COL_NAME: str, MESH_NAME_BASE: str, MESH_NAME_IDX_START: int,
                                                        Z_COORD_START: int, layer_name: str, with_symmetry: bool, layer_file_name: str,
                                                        symm_file_name: str, only_inside_border: bool, with_border: bool):
    #--- EDIT MODE!
    mesh_obj = bpy.context.active_object
    bm = bmesh.from_edit_mesh(mesh_obj.data)
    ...
    count, name = auto_strokes_nocross_inside_borders_with_optional_symmetry_with_layers(bm, starting_loop, Z_STEP, COL_NAME, MESH_NAME_BASE, MESH_NAME_IDX_START,
                                                                                         Z_COORD_START, layer_name, border_edges_id, accessable_faces_id, layer_dict,
                                                                                         only_inside_border, with_border, with_symmetry, symm_dict) 
...
class OBJECT_PT_FaceLoopsCurvesPanel(Panel):
    bl_label = "Face loops curves for strokes"
    ...
    def draw(self, context):
        layout = self.layout
        props = context.object.face_loops_curves
        
        col = layout.column()
        col.prop(props, "use_symmetry")
        
        col = layout.column()
        #spl = col.split()
        #box = col.box()
        col.prop(props, "use_auto")
        col.enabled = not props.use_auto_local
        col = layout.column()
        col.prop(props, "use_auto_local")
        col.enabled = not props.use_auto
        
        col = layout.column()
        col.prop(props, "use_edge_borders")

        col = layout.column()
        box = col.box()
        box.operator('object.show_edge_borders')
        box.operator('object.delete_edges_from_borders')
        box.operator('object.add_edges_to_borders')
        box.operator('object.clear_edge_borders')
        box.enabled = props.use_edge_borders

        col = layout.column()
        col.operator('object.face_loops_curves')        
    

# параметры для панели и для оператора (которые в функцию передаются)
class FaceLoopsCurvesProps(PropertyGroup):
    ...
    use_auto_local : BoolProperty(
        name = "Auto run inside call border area",
        default = False
    )

# оператор, т. е. вызов функции. Здесь вся логика
class FaceLoopsCurvesGenerator(Operator):
    '''
    Для выбранного объекта (в EDIT MODE)
    создает множество объектов StrokeMesh над uv разверткой,
    каждый Strokemesh содержит направляющие кривые, соответствующие
    обоъйденным за один вызов перпендикулярным кольцам.
    Все объекты помещаются в индексированные папки
    ...
    bl_idname = 'object.face_loops_curves'
    bl_label = 'Create curves for face loops'
    ...
    def get_params(self, context):
        props = context.object.face_loops_curves
        self.use_symmetry = props.use_symmetry
        self.use_auto = props.use_auto
        self.use_edge_borders = props.use_edge_borders
        self.use_auto_local= props.use_auto_local
        
    # собственно функция!
    def create_face_loops_curves(self):     
    ...
    def show_symmetry(self):
        ...
        mesh_obj = bpy.context.active_object
        bm = bmesh.from_edit_mesh(mesh_obj.data)
        symm_dict = make_symmetry_dictionary_by_median_similarity_with_central_strip(bm)

        central_faces = []
        pared_faces = []
        for key in symm_dict.keys():
            if symm_dict[key] == key:
                central_faces.append(key)
            else:
                pared_faces.append(key)

        #for id in central_faces:
        #    bm.faces[id].select = True
        for id in pared_faces:
            bm.faces[id].select = True
    ...
    def execute(self, context):
        ...
        self.create_face_loops_curves()
        #self.show_symmetry()
        
        return {'FINISHED'}

class AddEdgeBorderHandler(Operator):
    bl_idname = 'object.add_edges_to_borders'
    bl_label = 'Add selected edges to borders'
    ...
    def add_edges(self):
        ...
        selected_edges_id = [edge.index for edge in bm.edges if edge.select]

        LAYER_NAME_EDGE_IS_BORDER = "is_border_edge"
        write_edges_to_layer(mesh_obj, bm, LAYER_NAME_EDGE_IS_BORDER, selected_edges_id)
    ...
    def execute(self, context):
        ...
        #raise NotImplementedError
        return {'FINISHED'}
class DeleteEdgeBorderHandler(Operator):
    bl_idname = 'object.delete_edges_from_borders'
    bl_label = 'Delete selected edges from borders'
    ...
    def delete_edges(self):
        ...
        LAYER_NAME_EDGE_IS_BORDER = "is_border_edge"
        delete_edges_from_layer(mesh_obj, bm, LAYER_NAME_EDGE_IS_BORDER, selected_edges_id)
    ...
    def execute(self, context):
        ...
        #raise NotImplementedError
        return {'FINISHED'}    
class ShowEdgeBorderHandler(Operator):
    bl_idname = 'object.show_edge_borders'
    bl_label = 'Select edges from borders'
    ...
    def show_edges(self):
        ...
        LAYER_NAME_EDGE_IS_BORDER = "is_border_edge"
        show_select_all_edges_from_layer(bm, LAYER_NAME_EDGE_IS_BORDER)
    ...
    def execute(self, context):
        ...
        #raise NotImplementedError
        return {'FINISHED'}
class ClearEdgeBorderHandler(Operator):
    bl_idname = 'object.clear_edge_borders'
    bl_label = 'Clear edge borders'
    ...
    def clear_edges(self):
        ...
        all_edges_id = [edge.index for edge in bm.edges]
        LAYER_NAME_EDGE_IS_BORDER = "is_border_edge"
        delete_edges_from_layer(mesh_obj, bm, LAYER_NAME_EDGE_IS_BORDER, all_edges_id)
...
classes = [
    FaceLoopsCurvesProps,
    FaceLoopsCurvesGenerator,
    AddEdgeBorderHandler,
    DeleteEdgeBorderHandler,
    ShowEdgeBorderHandler,
    ClearEdgeBorderHandler,
    OBJECT_PT_FaceLoopsCurvesPanel
]
...
if __name__ == '__main__':
    ...
    bpy.types.Object.face_loops_curves = PointerProperty(type = FaceLoopsCurvesProps)
    
    # вызов вручную в скрипте: (потом будет кнопка, а в коде вызывать его не нужно)
    #bpy.ops.object.face_loops_curves()